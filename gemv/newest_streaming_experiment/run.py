...
# CONCURRENT STREAMING: A and x stream simultaneously
# Then b streams after x completes

# Stream A into all PEs - DON'T WAIT (nonblock=True)
# A streaming happens in background
...
# Stream x into top row - WAIT until complete (nonblock=False)
# A and x stream concurrently; host waits here for x to finish
runner.memcpy_h2d(MEMCPYH2D_DATA_1, x, 0, 0, kernel_x_dim, 1, N_per_PE, streaming=True,
  order=MemcpyOrder.ROW_MAJOR, data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)

# Stream b into left column - starts AFTER x completes
