...
def parse_instr_log(log_file_path):
    ...
    # We look for the task identifier (T followed by digits, or Task13.UT2 format)
    pattern = re.compile(r'@(\d+)\s+.*?Msg:\s+\[(?:IS|EX)\s+OP\].*?:\s+(T\d+(?:\.\w+)?|Task\d+(?:\.\w+)?)\s+')
...
def annotate_task(task_id):
    ...
    Handles task IDs like "T11" and "Task13.UT2".
    """
    # Check if task_id contains a dot (e.g., Task13.UT2)
    if '.' in task_id:
        parts = task_id.split('.')
        base_task = parts[0]
        ut_info = parts[1] if len(parts) > 1 else ''
        return f"{base_task}.{ut_info}"
